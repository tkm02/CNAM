...
<template>
  <div class="w-full h-screen flex justify-center items-center">
    <UCard :ui="{
      ...
      <UForm :state="state" :schema="useUserSchema" class="space-y-5" @submit="handleSubmit">
        <UFormGroup name="username">
          <UInput placeholder="Nom d'utilisateur" size="lg" icon="i-heroicons-user" v-model="state.username" />
        </UFormGroup>
        <UFormGroup name="password">
          <UInput type="password" placeholder="Mot de passe" size="lg" icon="i-heroicons-key"
            v-model="state.password" />
        </UFormGroup>
        <UButton label="Se connecter" block size="lg" type="submit" :loading="loading" />
      </UForm>
    </UCard>
  </div>
