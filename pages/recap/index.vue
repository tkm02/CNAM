        ...
        <tbody>
          <template v-for="region in stats" :key="region.region">
            <tr v-for="(equipe, index) in region.equipes" :key="`${region.region}-${equipe.nom}-${index}`">
              <td v-if="index === 0" :rowspan="region.equipes.length" class="region-cell">{{ region.region.toUpperCase() }}</td>
              <td v-if="index === 0" :rowspan="region.equipes.length" class="site-cell">{{ region.site_in_situ.toUpperCase() }}</td>
              ...
              <template v-for="date in dates" :key="`${equipe.nom}-${date}`">
                ...
                <td v-for="time in times" :key="`${equipe.nom}-${date}-${time}`">
...
function formatData(apiResponse: any) {
  ...
  apiResponse.forEach((regionData: any) => {
    ...
    regionData.equipe.forEach((equipeData: any) => {
      const { equipe, date, nb_op, nb_kit, nbr_imp, horaire, type,objectif,realise } = equipeData;
      ...
      if (existingStats) {
        ...
        existingStats.horaires[formattedHoraire] = (existingStats.horaires[formattedHoraire] || 0) ;
      } else {
