<template>
    ...
    {
    region: "LAGUNES",
    site_in_situ: "BINGERVILLE FOYER DES JEUNES",
    ...
    const equipes = staticData.stats.flatMap(stat => stat.equipes.map(equipe => equipe.nom));
    ...
    const worksheetData = [
    ["Région", "Site", "Activité", "Équipe", ...dates.value.flatMap(date => [
    `${date} Nb OP`,
    `${date} Nb KIT`,
    `${date} Nb IMP`,
    ...
    ...times.value.map(time => getStatByDateAndTime(equipe.stats, dates.value[0], time))
    ];
